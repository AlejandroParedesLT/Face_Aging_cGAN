...
#Added
#from evaluation_metrics_training import get_inception_model, calculate_fid, calculate_inception_score
...
flags.DEFINE_integer('max_steps', 2000, 'Number of batches to run')
...
#flags.DEFINE_string("source_file", './data/preprocessed-experiment-lists/train.txt', "source file path")
flags.DEFINE_string("source_file", './data/preprocessed-wiki-lists/train.txt', "source file path")

#flags.DEFINE_string("root_folder", 'D:/temporal_CV/1. AIPI 590 - Computer Vision/assignment/3. third_project/wiki_preprocess', "folder that contains images")
flags.DEFINE_string("root_folder", 'D:/temporal_CV/1. AIPI 590 - Computer Vision/assignment/3. third_project/wiki_preprocess/wiki', "folder that contains images")
...
def my_train():
    with tf.Graph().as_default():
        ...
        with open('training_losses.txt', mode='w') as file:
            file.write("Step|td_loss|tg_loss|tfea_loss|tage_loss\n")
            #sinception_model = get_inception_model()
            # Loop over max_steps
            for step in range(FLAGS.max_steps):
                ...
                file.write(f"{step}|{d_loss:.3f}|{g_loss:.3f}|{fea_loss:.3f}|{age_loss:.3f}\n")
                ...
                if step % VAL_INTERVAL == VAL_INTERVAL-1:
                    ...
                    for j in range(train_generator.n_classes):
                        ...
                        """
                        with open('evaluation_metrics.txt', mode='a') as eval_file:
                            print(type(source), source.shape if hasattr(source, 'shape') else "No shape")
                            print(type(samples), samples.shape if hasattr(samples, 'shape') else "No shape")
                            
                            source_np = source if isinstance(source, np.ndarray) else source.numpy()
                            samples_np = samples if isinstance(samples, np.ndarray) else samples.numpy()
                            # Calculate FID and Inception Score
                            fid = calculate_fid(source_np, samples_np, inception_model)
                            inception_mean, inception_std = calculate_inception_score(samples_np, inception_model)
                            # Log evaluation metrics
                            eval_file.write(f"{step}|{fid:.4f}|{inception_mean:.4f}|{inception_std:.4f}\n")
                            print(f"FID: {fid:.4f}, Inception Score: Mean = {inception_mean:.4f}, Std = {inception_std:.4f}")
                        """
def main(argv=None):
    my_train()
